function  output = MBMF_deterministic_2choices_p_sim(x,P)

% Mixed model-based / model-free simulation code for a task with
% deterministic transitions, two choices at the second level, and reward 
% probabilities for the second-level bandits
%
% USAGE: output = MBMF_deterministic_2choices_p_sim(x,rews)
%
% INPUTS:
...
for t = 1:N
    ...
    r = rand < P(t,s2,a(2));                                          % sample reward
    dtQ(2) = r - Q2(s2,a(2));                                         % prediction error (2nd choice)
